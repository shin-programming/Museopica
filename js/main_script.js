...
async function top_UI_prepare() {
    ...
    save_opt.addEventListener('click', () => {
        ...
        notes_data.forEach((notes, i) => {
            ...
            // 保存時のレイヤー補正：UIで layer が 0 の場合、保存するノーツの layer は 1 にする
            const noteCopy = Object.assign({}, notes);
            if (typeof layer !== 'undefined' && Number(layer) === 0) noteCopy.layer = 1;
            save_data.notes.push(noteCopy);
        });
    ...
    load_input.addEventListener('change', async () => {
        ...
        try {
            ...
            json_data.notes.forEach((notes, i) => {
                if (notes == null || notes == 'deleted') return;
                notes_data.push(notes);
                notes_data[i].id = i;
                // 旧バージョン互換：layerが無ければ1を割り当てる
                if (notes_data[i].layer == null) notes_data[i].layer = 1;
                Nplayed.push(false);
            });

            json_data.page_options.forEach((page, i) => page_options[i] = page);
            tempo = Number(json_data.page_options[0].tempo);
            document.getElementById('BPM_input').value = tempo;
            key = Number(json_data.page_options[0].key);
            document.getElementById('key_input').value = key;
            clone = notes_data.length;
    ...
    key_plus.addEventListener('click', () => {
        ...
        key_input.value++;
        key = Number(key_input.value);
        for (let i = 0; i < page_options.length; i++) page_options[i].key = key;
    });
    ...
    // レイヤーコントロール（0: 全表示, 1-10: 各レイヤー）
    const layer_input = document.getElementById('layer_input');
    const layer_label = document.getElementById('layer_label');
    layer = Number(layer_input.value);
    layer_label.textContent = layer;
    // CSS プロパティ初期化
    const layer_percent = (layer_input.value - layer_input.min) / (layer_input.max - layer_input.min) * 100;
    layer_input.style.setProperty('--layer-value', layer_percent + '%');

    layer_input.addEventListener('input', () => {
        layer = Number(layer_input.value);
        layer_label.textContent = layer;
        // スライダーのグラデーション反映
        const percent = (layer_input.value - layer_input.min) / (layer_input.max - layer_input.min) * 100;
        layer_input.style.setProperty('--layer-value', percent + '%');
        // レイヤー切替時に表示を更新
        left_bottom_ui_update();
        paint_notes_again();
    });
...
function play_music() {
    ...
    function play_loop() {
        ...
        for (let i = 0; i < notes_data.length; i++) {
            ...
            // ページとレイヤーでフィルタ（レイヤー0は全て再生）
            if (nd.start <= elapsed_time && !Nplayed[i] && nd.page == page && (layer === 0 || Number(nd.layer) === Number(layer))) {
                if (nd.key_type == 'D') {
                    // ドラム
                    play_drum(drums[nd.pitch - 1], nd.vol);
                } else {
                    // 白鍵 / 黒鍵
                    instruments[instruments.findIndex(row => row.name == nd.inst)].play(Number(nd.pitch) + key, audio_context_inst.currentTime, {
                        gain: Number(nd.vol),
                        duration: Number(nd.dur)
                    });
                }
        ...
        // 再計算：現在のレイヤーに表示される再生済みノーツ数を数える
        // played_notes = 0;
        // ページ移動でリセットしないため、総再生済みノーツ数（全ページ・全レイヤー）をカウント
        played_notes = Nplayed.filter(v => v === true).length;
...
function tempo_change() {
    for (let i = 0; i < page_options.length; i++) {
        // ページごとに変えるときの変更対象箇所
        page_options[i].tempo = tempo;
        page_options[i].change_time = 60 / tempo * 16;
    }
}
...
function left_bottom_ui_update() {
    // 表示レイヤーに基づくノーツ数算出（0は全表示)
    notes = notes_data.filter((v, i) => v !== 'deleted' && v != null && (layer === 0 || Number(v.layer) === Number(layer))).length;

    // 再生済みノーツ数はページ移動でリセットしない（全体の再生済みを表示）
    played_notes = Nplayed.filter(v => v === true).length;
...
function paint_notes_again() {
    ...
    notes_data.forEach((notes, index) => {
        if (notes == 'deleted') return;
        // レイヤーによる表示フィルタ（0 は全表示、1-10 は各レイヤーのみ）
        if (notes.page != page) return;
        if (!(layer === 0 || Number(notes.layer) === Number(layer))) return;
        ...
        if (
            ...
            pn_ctx.fillStyle = notes.key_type == 'B' ? `#${lighten_color(darken_color(notes.color.replace(/^#/, ''), 34), 34)}aa` : `#${lighten_color(notes.color.replace(/^#/, ''), 34)}aa`;
        } else {
            pn_ctx.fillStyle = notes.key_type == 'B' ? `#${darken_color(notes.color.replace(/^#/, ''), 34)}aa` : `#${notes.color.replace(/^#/, '')}aa`;
        }
...
function edit() {
    ...
    move_notes.addEventListener('click', () => {
        ...
        switch (mode) {
            case 'create':
                // 保存処理
                // UI が layer=0 の場合は新規ノーツは layer=1 にする
                const noteLayer = (typeof layer !== 'undefined' && Number(layer) === 0) ? 1 : layer;
                notes_data.push({
                    id: clone,
                    pos: {
                        x_num: xNum,
...
document.addEventListener('keydown', event => {
    if (event.shiftKey && event.key === 'P') toggle_playback_mode()
})

function toggle_playback_mode() {
    const app = document.getElementById('app');
    app.classList.toggle('playback_mode');

    // Force lock mode
    if (app.classList.contains('playback_mode')) {
        if (mode !== 'lock') {
            mode = 'lock';
            document.querySelectorAll('#mode_btns button').forEach(btn => btn.classList.remove('selected'));
            document.getElementById('lock_btn').classList.add('selected');
